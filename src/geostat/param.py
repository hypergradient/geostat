...
class Parameter:
    ...
    lo: float = np.nan
    hi: float = np.nan
    underlying: jax.Array = None  # Initialize as None, JAX will handle array creation
    ...
    def create_jax_variable(self):
        """Create JAX variable for underlying parameter or update it"""
        # Create the underlying parameter.
        b = self.bounding()
        if b == 'bb':
        ...
        if self.underlying is None:
            self.underlying = jnp.array(init, dtype=jnp.float32)
        else:
            self.underlying = jnp.array(init, dtype=jnp.float32)
    ...
    def surface(self):
        ...
        v = self.underlying
        if b == 'bb':
            v = jax.nn.sigmoid(v) * (self.hi - self.lo) + self.lo
        elif b == 'bu':
            v = jnp.exp(v) + self.lo
        elif b == 'ub':
            v = self.hi - jnp.exp(-v)
