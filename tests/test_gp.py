...
#     # Create parameters.
#     p = Parameters(alpha=1., range=0.33, nugget=1., sill=1.)
...
#            + krn.SquaredExponential(sill=p.sill, range=p.range) \
#            + krn.Noise(p.nugget)
#     model = Model(GP(0, kernel))
...
def test_gp3d():
    ...
    # Create parameters.
    p = Parameters(alpha=1., xscale=1., yscale=1., zscale=5., range=0.5, sill=1., gamma=1., dsill=0.1, nugget=0.1)
    ...
    kernel = \
        ...
        krn.GammaExponential(range=p.range, sill=p.sill, gamma=p.gamma, scale=[p.xscale, p.yscale, p.zscale]) + \
